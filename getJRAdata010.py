...
class ScrapeJRA():
    ...
    def next_page_date(self, year, month):
        ...
        cname = scrapejra.obj_param[(year + month)[2:]]
...
if __name__ == '__main__':
    ...
    exceldata = ExcelData()
    for i in range(len(exceldata.column_a)):
        exceldata.append_list("A", None)
    for i in range(len(exceldata.column_a)):
        exceldata.append_list("A", "test")
    for i in range(len(exceldata.column_b)):
        exceldata.append_list("B", "test")
    for i in range(len(exceldata.column_c)):
        exceldata.append_list("C", "test")
    for i in range(len(exceldata.column_c)):
        exceldata.append_list("C", "test")
    exceldata.create_new_excel(datetime.today(), datetime.today())
    ...
    # 例えば2020年6月を選択する
    scrapejra.next_page_date(2020, 6)
